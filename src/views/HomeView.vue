<script setup>
  ...
  const loadCardsFromPublishedCSV = async () => {
    if (!PUBLIC_CSV_URL) {
      console.log(
        'Chyba: PUBLIC_CSV_URL není nastavena. Zkontrolujte konfiguraci.'
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Nepodařilo se načíst CSV: ${response.status} ${response.statusText}. Zkontrolujte, zda je URL správná a list publikován.`
        );
      }
      ...
      if (parseResult.errors.length > 0) {
        ...
        throw new Error(
          `Chyba při parsování CSV: ${parseResult.errors[0].message}`
        );
      }
      ...
      if (dataRows.length === 0) {
        console.warn('CSV obsahuje prázdné řádky nebo je prázdný.');
        return;
      }
      ...
      if (
        ...
        // Basic check if first row looks like a header
        ...
        if (dataRows.length > 1) {
          // Only slice if there's more than one row (potential data beyond header)
      ...
      if (dataRows.length === 0) {
        error.value =
          'Nenalezena žádná data po případné hlavičce v CSV souboru.';
      ...
      const parsedCards = dataRows
        .map((columns) => {
          ...
          if (!Array.isArray(columns) || columns.length < 5) {
            console.warn(
              'Přeskakuji řádek s nedostatečným počtem sloupců:',
              columns
            );
      ...
      if (parsedCards.length === 0) {
        error.value =
          'Po parsování nebyly nalezeny žádné platné karty. Zkontrolujte formát CSV.';
        allCardsMasterList.value = [];
        deckOrder.value = [];
        isLoading.value = false;
        return;
  ...
  const shuffleDeck = () => {
    ...
    for (let i = allIdsToShuffle.length - 1; i > 0; i--) {
      ...
      [allIdsToShuffle[i], allIdsToShuffle[j]] = [
        allIdsToShuffle[j],
        allIdsToShuffle[i],
      ];
    }
  ...
  const drawCard = () => {
    if (deckOrder.value.length > 0) {
      ...
      console.log(
        'Card drawn. Hand size:',
        handCardIds.value.length,
        'Deck size:',
        deckOrder.value.length
      );
      saveState();
    } else {
  ...
  const completeCard = (cardId) => {
    ...
    if (cardIndexInHand > -1) {
      ...
      console.log(
        'Card completed. Completed size:',
        completedCardIds.value.length
      );
      saveState();
    }
  };
  ...
  const saveState = () => {
    try {
      localStorage.setItem(
        LS_ALL_CARDS_KEY,
        JSON.stringify(allCardsMasterList.value)
      );
      ...
      localStorage.setItem(
        LS_COMPLETED_IDS_KEY,
        JSON.stringify(completedCardIds.value)
      );
  ...
  const loadState = () => {
    try {
      ...
      if (
        ...
        handCardIds.value = handCardIds.value.filter((id) =>
          getCardDetails(id)
        );
        completedCardIds.value = completedCardIds.value.filter((id) =>
          getCardDetails(id)
        );
        return true;
      }
  ...
  onMounted(async () => {
    if (!loadState()) {
      console.log(
        'No valid state in LocalStorage or error loading, fetching from Google Sheets...'
      );
    ...
    if (
      allCardsMasterList.value.length === 0 &&
      !isLoading.value &&
      !error.value
    ) {
      error.value =
        'No cards available. Check Google Sheet configuration or LocalStorage.';
    }
  });

  // --- Computed Properties for Display ---
  const handCards = computed(() =>
    handCardIds.value.map(getCardDetails).filter(Boolean)
  );
  const completedCards = computed(() =>
    completedCardIds.value.map(getCardDetails).filter(Boolean)
  );
...
<template>
  ...
  >
    <div
      v-if="isLoading"
      class="text-slate-400 italic p-4 text-center w-full flex-1"
    >
      Načítání karet...
    </div>
    <div
    ...
    <div
      v-if="!isLoading && !error && allCardsMasterList.length === 0"
      class="text-slate-400 italic p-4 text-center w-full flex-1"
    >
    ...
    <div
      v-if="!isLoading && allCardsMasterList.length > 0"
      class="w-full h-full flex-1 overflow-y-auto"
    >
      <div class="space-y-8">
        <!-- Hand Zone -->
